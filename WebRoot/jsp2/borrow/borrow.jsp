...
<html lang="en">
  ...
  <!--<![endif]-->
    ...
    <div class="container-fluid">
                ...
                <div class="sidebar-nav">
                  <div class="nav-header" data-toggle="collapse" data-target="#dashboard-menu">图书馆</div>
                    <ul id="dashboard-menu" class="nav nav-list collapse in">
                        <li class="active"><a href="book_BookBorrowlist.action">尚学堂图书馆入口</a></li>
        ...
        <div class="span9">
   		<h1 class="page-title">欢迎来到尚学堂图书馆</h1>	
   		<a>当前图书馆有${page.totalCount }本书</a>
							...
							<c:forEach items="${list }" var="bean" varStatus="Status">
								<tr>
									...
									<td>${bean.name }</td>
								  	...
								  	<td>${bean.publish }</td>
								  	<td>${bean.pubDate }</td>
  	<td><a href="book_bookBorrow.action?record.bookId=${bean.id }&record.userId=${sessionScope.currentUser.id }&record.status=-1">借书</a></td>
...
<div class="pagination">
				<script type="text/javascript">
					...
					<ul>
						...
						<li><a
							href="javascript:pageFunction('book_BookBorrowlist.action?page.currentPage=${page.currentPage-1 }')">上一页</a></li>
						<c:choose>
							<%-- 只要总页数小于10直接显示所有页数 --%>
							<c:when test="${page.totalPage<10 }">
								<c:forEach begin="1" end="${page.totalPage }" var="pc" step="1">
									<li><a
										href="javascript:pageFunction('book_BookBorrowlist.action?page.currentPage=${pc }')">${pc }</a></li>
								</c:forEach>
							</c:when>
							<%-- 总页数大于10 --%>
							<c:otherwise>
						...
						<li><a
							href="javascript:pageFunction('book_BookBorrowlist.action?page.currentPage=${page.currentPage+1 }')">下一页</a></li>
					</ul>
				</div>
