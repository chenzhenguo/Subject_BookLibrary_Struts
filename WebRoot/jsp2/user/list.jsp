    ...
    <div class="container-fluid">
        ...
        <div class="row-fluid">
            <div class="span3">
                ...
                <ul id="dashboard-menu" class="nav nav-list collapse in">
         <li    class="active"> <a href="user_list.action">用户信息管理</a></li> 
                </ul>
...
<th>用户名id</th>
 ...
 <c:forEach items="${list }" var="bean">
    <tr>
  	...
  	<td>${bean.name }</td>
  	<td>${bean.password }</td>
  	<td><c:if test="${bean.roldId==1 }">管理员</c:if>
  	<c:if test="${bean.roldId==2 }">普通用户</c:if></td>
  	
  	<td><a href="user_delete.action?user.id=${bean.id }">删除</a>
  	<a href="user_toUpdate.action?user.id=${bean.id }">修改</a></td>
  </tr>
  </c:forEach>
						</tbody>
					</table>
